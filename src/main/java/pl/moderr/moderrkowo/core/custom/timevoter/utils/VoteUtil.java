package pl.moderr.moderrkowo.core.custom.timevoter.utils;
...
public class VoteUtil {
        ...
        if(timeVoter.lastVote == null || timeElapsed >= timeVoter.voteDelay){
            ...
            Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> {
                if(timeVoter.getYesVote().size() > timeVoter.getNoVote().size()){
                    ...
                    Main.getInstance().getServer().broadcastMessage(
                            ColorUtils.color(
                                    "&8[!] &cGłosowanie zostało zakończone niepowodzeniem." +
    ...
    private void createBossBar(){
        bossBar = Bukkit.createBossBar("Głosowanie", BarColor.BLUE, BarStyle.SOLID);
